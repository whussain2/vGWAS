...
### Draw the Linear Manhattan plot
...
```{r}
# First we will create a function to upload all the 3 csv files function
setwd("~/Dropbox/BoxUNL/Postdoc/data_analysis/Wheat_HWW_panel/vGWAS_output")
import.all<-function(mypath,mypattern,...)
{
  tmp.list.1<-list.files(mypath, pattern=mypattern)
  tmp.list.2<-list(length=length(tmp.list.1))
...
# Now upload the all the files
csv.import<-import.all("~/Dropbox/BoxUNL/Postdoc/data_analysis/Wheat_HWW_panel/vGWAS_output", "csv$",sep=",")
# here we define the separator of entries in the csv files to be comma.
gwas_cd<-csv.import$gwas_cd.csv
dglm_cd<-csv.import$dglm_cd.csv
hglm_cd<-csv.import$hglm_cd.csv
...
library(qqman)
      ...
      my.manhattan<-function(data,chr, bp, p, snp){
        manhattan(x = data, chr, bp, p, snp, suggestiveline=FALSE, genomewideline= -log10(1.266464e-05), chrlabs =c("1A", "1B", "1D", "2A", "2B", "2D","3A", "3B", "3D","4A", "4B", "4D","5A", "5B", "5D","6A", "6B", "6D", "7A", "7B", "7D", "UN"), col = c("blue", "orange", "skyblue", "darkred"), logp = TRUE, cex.lab=1.5, ylim=c(0,8), xlab="Chromosome", ylab="-log10(p)", cex.axis = 0.8, mgp=c(2,1,0))
        
      }
      png(file="~/Dropbox/BoxUNL/Postdoc/data_analysis/Wheat_HWW_panel/vGWAS_output/Manhattan_plot_all.png", width=7, height =9, units = 'in',res=300)
      par(mfrow=c(3,1))
      my.manhattan(all_p, chr="chrom", bp="pos", p="gwas_p", snp="marker")
      my.manhattan(all_p, chr="chrom", bp="pos", p="dglm_p.disp", snp="marker")
      my.manhattan(all_p, chr="chrom", bp="pos", p="hglm_p.disp", snp="marker")
      dev.off()
```
## **Violin plots**
...
epis_pvalues<-read.csv(file="~/Dropbox/BoxUNL/Postdoc/data_analysis/Wheat_HWW_panel/Manuscript_data/Epistasis_HM_MANUSCRIPT/epistasis_pvalues.csv",
...
# Draw the heat map
png(file="~/Dropbox/BoxUNL/Postdoc/data_analysis/Wheat_HWW_panel/Manuscript_data/Epistasis_HM_MANUSCRIPT/epistasis_heatmap.png",
    width=12, height =9, units = 'in',res=400)
pheatmap(epis_pvalues1,  clustering_distance_rows = "correlation",clustering_distance_cols="correlation", scale = "none", legend_labels = "-log10(p-value", 
         fontsize=13, angle_col = 45)
dev.off()
